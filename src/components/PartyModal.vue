<template>
  <Modal v-model:show="showModal" title="Manage your parties and players">
              ...
              <div v-show="party.editing" class="border-x border-gray-50 dark:border-gray-700 dark:bg-gray-800 flex flex-col gap-x-1 gap-y-2 px-6 py-3">
                ...
                <div v-for="(player, playerIndex) of party.players">
                  <div class="grid gap-2 grid-cols-[60px_1fr_1fr] md:grid-cols-[60px_1fr_50px_75px_150px_30px]">
                    ...
                    <div class="order-3">
                      <input type="number" :id="'level_'+playerIndex" v-model="player.level" class="px-1 py-1 block w-full sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 focus:ring-emerald-500 focus:border-emerald-500 border-gray-300">
                    </div>

                    <div class="relative order-5 md:order-4 mb-2 md:mb-0">
                      <input type="number" :id="'initiativeMod_'+playerIndex" v-model="player.initiativeMod" class="pl-1 pr-8 py-1 block w-full sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 focus:ring-emerald-500 focus:border-emerald-500 border-gray-300">
                    ...
                    <div class="order-6 md:order-5 justify-center md:justify-start flex -space-x-px mb-2 md:mb-0">
                      <div class="w-1/2 flex-1 min-w-0">
                        <input min="0" @change="playerChange(player)" @blur="playerChange(player)" type="number" v-model="player.currentHp" class="px-1 py-1 text-right border-r-0 relative block w-full rounded-none rounded-l-md sm:text-sm dark:bg-gray-700 dark:border-gray-600 focus:ring-emerald-500 focus:border-emerald-500 border-gray-300"/>
                      </div>
                      ...
                      <div class="flex-1 min-w-0">
                        <input min="1" type="number" v-model="player.maxHp" class="px-1 py-1 relative border-l-0 block w-full rounded-none rounded-r-md sm:text-sm dark:bg-gray-700 dark:border-gray-600 focus:ring-emerald-500 focus:border-emerald-500 border-gray-300"/>
                      </div>
                    </div>
    ...
    <template #footer>
      <button @click="showModal = false" type="button" class="button-primary-md">Close</button>
...
function activateParty(partyIndex) {
  console.log('would have activated party' + partyIndex);
}

function deactivateParty(partyIndex) {
  console.log('would have dactivated party ' + partyIndex);
}

function createPlayer(partyIndex) {
  console.log("would have created player on index " + partyIndex);
}

function playerChange(value) {
  console.log("caught player change ", value);
}

function deletePlayer(partyIndex) {
  console.log("would have deleted player on index " + partyIndex);
}

function createParty() {
  console.log("would have created a new party");
}

function deleteParty(partyIndex) {
  console.log('would have deleted party at index '+ partyIndex);
}
